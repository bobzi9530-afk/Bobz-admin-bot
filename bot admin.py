...
MAT_BAD_WORDS = [
    "хуй".upper().lower(),"бля".upper().lower(),"блять".upper().lower(),"хуйй".upper().lower(),"бляя".upper().lower(),"блят".upper().lower(),
    "хууй".upper().lower(),"бляяя".upper().lower(),"блятb".upper().lower(),"ххуй".upper().lower(),"бляяяя".upper().lower(),"блят".upper().lower(),
    "хyй".upper().lower(),"blya".upper().lower(),"бляTb".upper().lower(),"блyть".upper().lower(),"blyat".upper().lower(),"bliat".upper().lower(),
    "хYI".upper().lower(), "blya".upper().lower(), "бляt".upper().lower(), "блaть".upper().lower(),"xyi".upper().lower(), "xyи".upper().lower(),
    "xYI".upper().lower(), "blyATM".upper().lower(), "бляtv".upper().lower(), "бл0ть".upper().lower(),"бл#ть".upper().lower(), "x@й".upper().lower(),
    "пидорас".upper().lower(), "пидора".upper().lower(), "пидор".upper().lower(), "pidor".upper().lower(),
    "pidoras".upper().lower(), "pidora".upper().lower(), "пидopас".upper().lower(), "пидорa".upper().lower(), "пидoр".upper().lower(), "пидoрас".upper().lower(),
    "пидоpас".upper().lower(), "пiдор".upper().lower(), "пидоpoс".upper().lower(), "педорас".upper().lower(),     "шлюха".upper().lower(), "хуйня".upper().lower(),
    "шлюшка".upper().lower(), "херня".upper().lower(), "хуйло".upper().lower(), "хуiло".upper().lower(), "хуило".upper().lower(), "хуйлo".upper().lower(),
    "хуiло".upper().lower(), "xyйлo".upper().lower(), "хyйло".upper().lower(), "xуйло".upper().lower(), 'ухуяшить''ухуяшиться''ухуяшивать'
    'ухуяшиваться'.upper().lower(),'фаллос'.upper().lower(),'фекал'.upper().lower(),
    'фекалий'.upper().lower(),'фекалии'.upper().lower(),'хер'.upper().lower(),
    'херами'.upper().lower(),'херня'.upper().lower(),'херовина'.upper().lower(),
    'херов'.upper().lower(),'хрен'.upper().lower(),'хреново'.upper().lower(),
    'хреновое'.upper().lower(),'хреновый'.upper().lower(),'хуевина'.upper().lower(),
    'хуев'.upper().lower(),'хуево'.upper().lower(),'хуевый'.upper().lower(),
    'хуек'.upper().lower(),'хуечек'.upper().lower(),'худоебина'.upper().lower(),
    'хуебень'.upper().lower(),'хуев'.upper().lower(),'хуева'.upper().lower(),
    'хуевато'.upper().lower(),'хуеватый'.upper().lower(),'хуеглот'.upper().lower(),
    'хуегрыз'.upper().lower(),'хуедрыга'.upper().lower(),'хуемудрие'.upper().lower(),
    'хуемыслие'.upper().lower(),'хуеньки'.upper().lower(),'хуеплет'.upper().lower(),
    'хуесос'.upper().lower(),'хуета'.upper().lower(),'хуетень'.upper().lower(),
    'хуец'.upper().lower(),'хуила'.upper().lower(),'хуиный'.upper().lower(),
    'хуистый'.upper().lower(),'хуишко'.upper().lower(),'хуище'.upper().lower(),
    'хуи'.upper().lower(),'хуило'.upper().lower(),'хуйло'.upper().lower(),
    'хуй'.upper().lower(),'хуйство'.upper().lower(),'хуйнуть'.upper().lower(),
    'хуйня'.upper().lower(),'хуйню'.upper().lower(),'хули'.upper().lower(),'хуюжить'.upper().lower(),
     'хуюжиться'.upper().lower(), 'хуюживать'.upper().lower(), 'хуюживаться'.upper().lower(), 'хуюшки'.upper().lower(),
     'хуя'.upper().lower(), 'хуяк'.upper().lower(),'хуякать'.upper().lower()
...
def _links_save_data_safe():
    try:
        if callable(globals().get("save_data")):
            save_data()
    except Exception as e:
        print(f"links save_data error: {e}")


def _links_init_storage():
    data.setdefault("links_filter", {})
    data["links_filter"].setdefault("chats", {})


def _links_get_chat_settings(chat_id):
    _links_init_storage()

    chat_id = str(chat_id)

    if chat_id not in data["links_filter"]["chats"]:
        data["links_filter"]["chats"][chat_id] = {
            "allowed": True
        }

    settings = data["links_filter"]["chats"][chat_id]
    settings.setdefault("allowed", True)

    return settings


def _links_are_allowed(chat_id):
    settings = _links_get_chat_settings(chat_id)
    return bool(settings.get("allowed", True))


def _links_set_allowed(chat_id, allowed):
    settings = _links_get_chat_settings(chat_id)
    settings["allowed"] = bool(allowed)
    _links_save_data_safe()


def _links_is_chat_admin(chat_id, user_id):
    try:
...
def _links_can_manage(message):
    if not message.from_user:
        return False

    if callable(globals().get("_is_admin_id")):
        try:
    ...
    if message.chat.type in ["group", "supergroup"]:
        return _links_is_chat_admin(message.chat.id, message.from_user.id)

    return False


def _links_user_is_admin(message):
    if not message.from_user:
        return False
...
def _links_get_message_text(message):
    return getattr(message, "text", None) or getattr(message, "caption", None) or ""


def _links_has_link_entities(message):
    entities = []
...
def _links_contains_link(message):
    text = _links_get_message_text(message)

    if _links_has_link_entities(message):
        return True

    if not text:
        return False

    return bool(LINKS_URL_RE.search(text))


def _links_delete_message(message):
    try:
        bot.delete_message(message.chat.id, message.message_id)
        return True
    except Exception as e:
        print(f"links delete error: {e}")
        return False
...
def _mat_get_normalized_bad_words():
    words = []

    for word in MAT_BAD_WORDS:
        normalized_word = _mat_normalize_text(word)
...
def _mat_contains_bad_word(text):
    normalized_text = _mat_normalize_text(text)

    if not normalized_text:
        return False

    bad_words = _mat_get_normalized_bad_words()
    ...
    for bad_word in bad_words:
        # Если в списке обычное одно слово
        if " " not in bad_word:
            if bad_word in text_words:
                return True

        # Если в списке фраза из нескольких слов
        else:
            normalized_text_with_spaces = f" {normalized_text} "
            bad_phrase_with_spaces = f" {bad_word} "

            if bad_phrase_with_spaces in normalized_text_with_spaces:
                return True
...
def cmd_now(message):
    text = (
        ...
        "• -ссылки - включить фильтр на ссылки\n"
        "• +ссылки - выключить фильтр на ссылки\n"
    )
...
@bot.message_handler(
    content_types=["text"],
    func=lambda m: bool(getattr(m, "text", None)) and _mat_is_enabled(m.chat.id) and _mat_contains_bad_word(m.text)
)
def mat_filter_handler(message):
    print("СРАБОТАЛ АНТИМАТ:", message.text)
...
@bot.message_handler(
    ...
    func=lambda m: not _links_are_allowed(m.chat.id) and _links_contains_link(m)
)
def links_filter_handler(message):
    print("СРАБОТАЛИ АНТИСЫЛКИ")
    ...
    if _links_user_is_admin(message):
        return

    deleted = _links_delete_message(message)

    if deleted:
        bot.send_message(
            message.chat.id,
            "🚫 Ссылки запрещены в этом чате.",
            parse_mode=None
        )
...
def track_all_messages(message):
    print("ОБЩИЙ ОБРАБОТЧИК:", message.text)

    # твой код статистики / активности / RP и т.д.

@bot.message_handler(func=lambda m: bool(getattr(m, "text", None)) and m.text.strip().lower() == "+ссылки")
def cmd_links_enable(message):
    if not is_admin(message.chat.id, message.from_user.id):
        bot.reply_to(message, "Удалять сообщения могут только администраторы.", parse_mode=None)
        return

    if not _links_can_manage(message):
        return

    _links_set_allowed(message.chat.id, True)

    bot.reply_to(
        message,
        "✅ Ссылки разрешены в этом чате.",
        parse_mode=None
    )


@bot.message_handler(func=lambda m: bool(getattr(m, "text", None)) and m.text.strip().lower() == "-ссылки")
def cmd_links_disable(message):
    if not is_admin(message.chat.id, message.from_user.id):
        bot.reply_to(message, "Удалять сообщения могут только администраторы.", parse_mode=None)
        return
    ...
    _links_set_allowed(message.chat.id, False)

    bot.reply_to(
        message,
        "🚫 Ссылки запрещены в этом чате.\n"
        "Теперь бот будет удалять сообщения со ссылками.",
        parse_mode=None
    )
...
def links_filter_handler(message):
    ...
    if deleted:
        try:
            ...
            print(f"links warning send error: {e}")
